...
HF free Spaces expose **one port (7860)** and have **16 GB RAM** with strict **CPU limits** (sustained 100% CPU can pause the Space). This hub uses **lazy launch**: only **one frontend runs at a time**; idle apps are stopped to avoid VNC-style CPU spikes. Switch from `/hub` or open an app URL directly.
...
5. **Shortcuts:** `/api/switch/lumiverse` etc. start that app and stop idle ones. First open can take **1–3 minutes**. Set `HUB_LAUNCH_MODE=always-on` only on paid hardware if you need all three parallel.
